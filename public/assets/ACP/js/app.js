...
var App = function() {
    ...
    var uiInit = function() {
        ...
        CKEDITOR.plugins.addExternal('cmsimage',basePath + 'ckeditor-plugins/cmsimage/', 'plugin.js');
        CKEDITOR.plugins.addExternal('uploadimage',basePath + 'ckeditor-plugins/uploadimage/', 'plugin.js');

        $('.textarea-editor').ckeditor({
            uploadUrl : '/acp/laravel-filemanager/uploadDD?type=Images&_token=' + _csrf_token,
            filebrowserImageBrowseUrl: '/acp/laravel-filemanager?type=Images',
            filebrowserImageUploadUrl: '/acp/laravel-filemanager/upload?type=Images&_token=' +  _csrf_token,
            //filebrowserBrowseUrl: '/acp/laravel-filemanager?type=Files',
            //filebrowserUploadUrl: '/acp/laravel-filemanager/upload?type=Files&_token=' + _csrf_token,
            allowedContent: true,
            extraPlugins: 'cmsimage',
            language: 'it',
            toolbarGroups: [
                ...
                { name: 'styles', groups: [ 'colors', 'basicstyles', 'cleanup' ] }
            ],
            removeButtons: 'Subscript,Superscript,Image,Flash,HorizontalRule,PageBreak,Styles,Anchor,Smiley,Iframe,PasteText,Paste',
        ...
        $(".pick-a-color").each(function(){
            $(this).spectrum({
                showPalette: true,
                palette: $(this).data('palette-color')?$(this).data('palette-color').split(','):[],
                //showSelectionPalette: false, // true by default
        ...
        $(".icon-picker").each(function(){
            ...
            var field = '[name=' + target.replace('[', '\\\[').replace(']', '\\\]').replace('@', '\\\@') + ']';
            field = $(field);
            field.hide();

            othis.data('icon', field.val());


            othis.iconpicker().on('change', function(e){
                field.val(e.icon);
            });
        });
        ...
        $(".file-picker").each(function(){
            var othis = $(this);
            var html = '';
            html += '<input class="form-control" type="button" onclick="cms_ipopup(\'Immagine\',\'/acp/laravel-filemanager?type=Images&CKEditor=' + othis.attr('name') + '&CKEditorFuncNum=cms_useFile&langCode=en&ipopup=1\');" />';
            ...
            html +=
                ...
                +'<img style="max-width:100%" src="' + othis.data('preview') + '" class="file-preview-image">'
                +'<input type="hidden" name="' + othis.attr('name') + '" value="' + othis.val() + '" />'
                +'</div>'
                +'</div>'
                ...
                +'<div class="file-caption-name" title=" "><span class="glyphicon glyphicon-file kv-caption-icon"></span> </div>'
                +'</div>'
                ...
                +'<div onclick="cms_ipopup(\'Immagine\',\'/acp/laravel-filemanager?type=Images&CKEditor=' + othis.attr('name') + '&CKEditorFuncNum=cms_useFile&langCode=en&ipopup=1\');" tabindex="500" class="btn btn-primary btn-file"><i class="glyphicon glyphicon-folder-open"></i> <span class="hidden-xs">Cambia Imagine</span></div>'
                +'</div>'
                +'</div>'
                +'</div>';
...
function cms_useFile(field, value){
    console.log(['>>> received', field, value]);
    var field = field.replace('[', '\\[').replace(']', '\\]').replace('@', '\\@');
    console.log(field);
    $('input[name=' + field + ']').val(value);
    $('input[name=' + field + ']').parent('div').find('img').attr('src', value);
}
function cms_useFileInCK(field, value){
    console.log(['>>> received', field, value]);
    console.log(field);
...
function cms_popup(url) {
    newwindow=window.open(url,'name','height=600,width=800');
    if (window.focus) {newwindow.focus()}
    return false;
}

function cms_ipopup(title, url){

    $('#cmsIPopup iframe').attr("src","");
    setTimeout(function(){
        $('#cmsIPopup iframe').attr("src",url);
    },200);
    $('#cmsIPopup h2').html(title);
    $('#cmsIPopup').modal({show:true})
}

//window.open = function(a,b,c){
//    $('#cmsIPopup iframe').attr("src","");
//    $('#cmsIPopup h2').html("Immagine");
//    $('#cmsIPopup').modal({show:true});
//
//    return $('#cmsIPopup iframe')[0].contentWindow;
//}

function transformFileInput(elm)
{
    var config = {
        showPreview : true,
        showRemove : false,
        ...
        initialCaption: ' ',//'Select JPG/PNG/GIF file...'
    };
    var preview = $(elm).data('preview');
    if(preview != '')
    {
        config.initialPreview = ["<img style='max-width:100%' src='" + preview + "' class='file-preview-image'>"];
    }

    $(elm).fileinput(config).on('fileclear', function(event) {
        // $(elm).fileinput('destroy');
        // transformFileInput(elm);
    });
}
